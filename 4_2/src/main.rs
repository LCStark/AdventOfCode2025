...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    loop {
        let mut sum = 0;
        let mut can_remove: Vec<(i32, i32)> = vec![];
        for y in 0..array.len() {
            let row = &array[y];
            for x in 0..row.len() {
                if array[y as usize][x as usize] != '@' { continue; } 
                let count = count_neighbours(&array, x as i32, y as i32);
                if count < 4 { sum += 1; can_remove.push((x as i32, y as i32))};
            }
        }
        for n in 0..can_remove.len() {
            array[can_remove[n].1 as usize][can_remove[n].0 as usize] = 'X';
        }
        total_sum += sum;
        if sum == 0 { break; }
    }
    
    println!("Total sum: {0}", total_sum);
